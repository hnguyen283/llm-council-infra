@echo off
REM ============================================================================
REM laptop-local-ai.bat -- Run the laptop side of the prod-lite deployment.
REM
REM Starts:
...
if "%~1"=="" (
    set "OPTION=prod-lite-vps-hybrid"
) else (
    set "OPTION=%~1"
)
...
if not exist "%ENV_FILE%" (
    ...
    echo Create options\%OPTION%\.env and set the prod-lite local/VPS runtime values.
    exit /b 1
)
...
:help
echo Usage: scripts\laptop-local-ai.bat [prod-lite-vps-hybrid]
echo.
echo Starts local knowledge database, Ollama, and local-ai-service using options\prod-lite-vps-hybrid\.env.
